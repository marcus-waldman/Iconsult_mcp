...
**Status (as of 2026-05-01):** **Phases 1-5 complete. Phase 6 (final merge gate) remains.** All shipped on `feat/multi-book-kg` and pushed to `origin`. Phase 2 verification: `arsanjani_2026.summary` embedded as 1536-dim vector; `triage_books` cleanly separates relevant from off-topic. Phase 3a: schema + 11 helpers + `list_books` (27th tool). Phase 3b: `start_project(...)` (28th tool) with internal triage fallback and deterministic project_id. **Inline ingestion:** `gulli_2025` (Antonio Gulli, *Agentic Design Patterns*, Springer 2025, altitude=`implementation`) — 180 concepts / 390 sections / 802 relationships / summary embedded; concepts schema migrated to `UNIQUE(name, book_id)` so the 5 cross-book name collisions coexist; triage cleanly separates Gulli-leaning from Arsanjani-leaning queries. **Phase 3c verification:** `align_book_pair.py` + `build_project_kg(project_id)` MCP tool (29th tool); `align_book_pair` adjudicated 94 candidate pairs at cosine threshold 0.6/top-5 per side via Sonnet 4 in 8 batches, yielding 48 same-concept verdicts cached in `concept_alignment_cache`; `build_project_kg` clusters via union-find, picks canonical names (oracle-preferred), classifies role (40 supporting_evidence + 233 informational_only on a 318-concept two-book project → 273 clusters, 26 multi-member). Manually reviewed sample of multi-member clusters confirms quality (e.g., "Agent-to-Agent (A2A)" rolls up 5 concept variants across both books anchored to rubric `agent_delegates_to_agent`; "Hybrid Model" pulls Arsanjani's Supervisor variants together with Gulli's Hierarchical Structures under rubric `supervisor_architecture`). **Phase 4a verification:** `consultations.project_id` column added (idempotent ALTER TABLE migration); new `db.search_canonical_concepts_by_embedding` helper; `match_concepts` accepts optional `project_id` and routes to the canonical layer when the project's KG is built (returns canonical IDs with `member_concept_ids` / `role` / `rubric_pattern_id`), persists `project_id` on the consultations row, falls back cleanly when omitted; errors when project unknown or KG not built. 206/206 tests pass (7 new in `test_match_concepts_project_scoped.py`). **Phase 4b verification:** new `db.get_canonical_subgraph` helper performs canonical-cluster traversal — each canonical seed expands to its source-book members, BFS runs across raw `relationships`, results collapse back to canonical clusters via **option 1 max-confidence collapse** (one canonical edge per `(from_canonical, to_canonical)` pair, highest-confidence source edge wins; relationship_type comes from the winner; intra-cluster source edges are filtered). `get_subgraph` tool gains optional `project_id` plus auto-pickup from `consultations.project_id` so callers don't re-pass it. Nodes carry `member_concept_ids` / `role` / `rubric_pattern_id` when project-scoped. 215/215 tests pass (9 new in `test_get_subgraph_project_scoped.py`). **Phase 4c verification:** `search_sections_by_embedding` extended with optional `book_ids` filter (additive; sections now carry `book_id` in returned dict). `ask_book` accepts optional `project_id` plus auto-pickup from the consultation row. When project-scoped: caller-supplied canonical `concept_ids` are expanded to source-book members via `_expand_canonical_to_members`; section search filters by `(expanded_members, triaged_book_ids)`; passages carry `book_id` provenance; `expanded_member_concept_ids` exposed in the response; suggested questions derive from expanded members. 224/224 tests pass (9 new in `test_ask_book_project_scoped.py`). **Phase 4d verification:** `log_pattern_assessment` accepts optional `source_book_id` and `canonical_concept_id` provenance fields. Pure attribution: persisted on `step_data`, returned in the response, surfaced through `_get_pattern_assessments` and `db.get_pattern_assessments` so failure_scenarios / render_report inherit them automatically. No validation, no behaviour change to score_architecture (still keys on `pattern_id` resolved through rubric aliases — verified by an invariance test that scores plain vs. provenance-tagged assessments identically). 230/230 tests pass (6 new in `test_log_pattern_assessment_provenance.py`). **Phase 4e verification:** end-to-end live walkthrough via `scripts/verify_phase4e.py` against arsanjani_2026 + gulli_2025. All seven stages (start_project → build_project_kg → match_concepts → get_subgraph → ask_book → log_pattern_assessment with provenance → score_architecture) execute cleanly. Top match for an agentic-systems query is "Hybrid Model" — a 4-member cross-book canonical cluster anchored to rubric `supervisor_architecture`. `get_subgraph` returns 119 canonical nodes / 12 edges (truncated from 297) all keyed on `{project_id}__*` IDs. `ask_book` correctly expands canonical → members across both books (4 expanded members including `gulli_2025__hierarchical_structures`) and scopes passages to triaged_book_ids with `book_id` provenance per passage. A `gulli_2025`-sourced pattern_assessment routes cleanly through the rubric and lands on the Coordination & Planning scorecard. The `auto_healing_pattern` ID normalised through `_PATTERN_ID_ALIASES` to `auto_healing_agent_resuscitation` as expected from the Phase 3c alias-gap follow-ups. **Phase 4 plumbing complete.** **Phase 5 verification:** consumption pass over the provenance written by Phase 4d. `score_architecture` (5a) surfaces `source_book_id` / `canonical_concept_id` on each per-pattern entry plus an optional `overall_summary["by_source_book"]` rollup; `generate_failure_scenarios` (5b) carries `source_book_id` on every scenario (defaulting to `arsanjani_2026` for book-grounded templates since the rubric IS arsanjani Ch. 12); `render_report` (5c) renders subtle gray `[book_id]` badges in the scorecard and stress-test sections, gated on the multi-book signal so legacy / single-book reports stay byte-identical. Live walkthrough via `verify_phase4e.py` shows `overall_summary["by_source_book"] = {"arsanjani_2026": 2, "gulli_2025": 1}` after a 3-assessment cross-book run; `verify_phase5c.py` renders the same consultation through `render_report` and confirms badges land at the right rows (unassessed siblings have no phantom badges). 245/245 tests pass (5 + 5 + 5 = 15 new across `test_score_architecture_provenance.py`, `test_failure_scenarios_provenance.py`, `test_render_report_provenance.py`). **Phase 5 complete.** Next: Phase 6 (final merge gate — real consultation comparison vs single-book baseline).
...
**Phase 5 sub-staging — done.** Four reviewable sub-stages on `feat/multi-book-kg`. Briefing at [`todo/phase-5-briefing.md`](./todo/phase-5-briefing.md). Phase 5 was the consumption pass over the provenance fields Phase 4d wrote — three downstream consumers (score_architecture, failure_scenarios, render_report) inherited the new fields for free via the field-agnostic `_get_pattern_assessments` reader, so each sub-stage was small, additive, and backwards-compatible by construction.

| Stage | Commit | Scope | Verification |
|---|---|---|---|
| **5a** | `38eeb0b` | `score_architecture`: each entry in `categories[k]["levels"][lv]["patterns"]` carries optional `source_book_id` / `canonical_concept_id` from the matching `pattern_assessment` step. `overall_summary["by_source_book"]` rollup counts assessed rubric patterns per source book, emitted only when at least one assessment had provenance (option 1 minimal — no per-category rollup). Aliases dedupe so a book-prefixed `gulli_2025__supervisor_architecture` rolls up under one count for the canonical rubric pattern | ✅ 235/235 tests pass; 5 new in `test_score_architecture_provenance.py`; legacy / single-book consultations produce byte-identical output to pre-5a |
| **5b** | `b1d7405` | `generate_failure_scenarios`: every scenario gains `source_book_id`. User-supplied `source_book_id` on the matching assessment wins; otherwise default to `arsanjani_2026` (5b-Q1 option 1: the rubric IS arsanjani Ch. 12, every `PATTERN_FAILURE_TEMPLATES` entry sources from Ch. 7-12, so book-grounded scenarios are arsanjani-sourced by construction). Each scenario gets a consistent badge regardless of mode; the existing `mode` field still distinguishes code-grounded vs book-grounded for callers that need it | ✅ 240/240 tests pass; 5 new in `test_failure_scenarios_provenance.py`; aliased pattern_ids retain provenance through `normalize_pattern_id` |
| **5c** | `b53bce2` + `c08d232` | `render_report`: provenance badges (`<span class="book-badge">[gulli_2025]</span>` — subtle gray monospace, font-size 10px, opacity 0.8) render next to assessed pattern names in the scorecard and next to scenario titles in the stress-test section. Single `show_book_badges` flag derived from `score_data["overall_summary"]["by_source_book"]` gates whether ANY badges render — legacy / single-book reports stay byte-identical (the `.book-badge` CSS class always ships in the template, but no `<span>` instances render). 5c-Q1 locked: subtle gray monospace per briefing recommendation. Roadmap canonical-name lookup deferred — not needed for the primary attribution signal. `c08d232` adds `scripts/verify_phase5c.py` for live visual smoke testing and a stress-test specific badge unit test | ✅ 245/245 tests pass; 5 new in `test_render_report_provenance.py` (legacy emits no badges, multi-book emits both books' badges, unassessed siblings get no phantom badge, stress-test scenario carries badge inside `<details class="scenario">`, CSS class always present); live render at `~/.agent/diagrams/consultation-447b5461c75e_*.html` visually confirmed |
| **5d** | (this commit) | Phase 5 closure: re-run `verify_phase4e.py` (`overall_summary["by_source_book"] = {"arsanjani_2026": 2, "gulli_2025": 1}` live), render the multi-book demo consultation through `render_report` and visually inspect badges, update CLAUDE.md (Key Commands + Testing + score_architecture / generate_failure_scenarios / render_report MCP Tools entries), update this plan's Implementation Tracking, push | ✅ 245/245 tests pass; provenance round-trips intact; Phase 6 remaining |
