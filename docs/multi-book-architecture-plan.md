...
**Status (as of 2026-04-30):** **Phases 1 and 2 complete; Phase 3a complete.** All sub-stages (1a–1d, 2a–2c) shipped on `feat/multi-book-kg` and pushed to `origin`. Phase 2 verification: `arsanjani_2026.summary` (5145 chars, hybrid Claude-draft + user edit) embedded as 1536-dim vector; `triage_books` MCP tool returns 0.4502 cosine for an obviously-relevant agentic project description and < 0.2 for off-topic ones; 142/142 tests pass. Phase 3a verification: schema (`projects`, `canonical_concepts`, `concept_alignment_cache`) + 11 db helpers + `list_books` MCP tool (27th tool); 23 new tests, 165/165 total passing. Next: Phase 3b (`start_project` tool) — see [`todo/phase-3-briefing.md`](./todo/phase-3-briefing.md).
