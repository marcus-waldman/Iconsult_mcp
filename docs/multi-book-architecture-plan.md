...
**Status (as of 2026-04-30):** **Phases 1 and 2 complete; Phases 3a and 3b complete.** All sub-stages (1a–1d, 2a–2c) shipped on `feat/multi-book-kg` and pushed to `origin`. Phase 2 verification: `arsanjani_2026.summary` (5145 chars, hybrid Claude-draft + user edit) embedded as 1536-dim vector; `triage_books` MCP tool returns 0.4502 cosine for an obviously-relevant agentic project description and < 0.2 for off-topic ones; 142/142 tests pass. Phase 3a verification: schema (`projects`, `canonical_concepts`, `concept_alignment_cache`) + 11 db helpers + `list_books` MCP tool (27th tool); 23 new tests, 165/165 total passing. Phase 3b verification: `start_project(name, project_description, triaged_book_ids?, project_id?)` MCP tool (28th tool) with internal triage fallback and deterministic project_id derivation; 12 new tests, 177/177 total passing. **Next: inline second-book ingestion** before 3c (alignment) — pick from Hohpe EIP / Nygard *Release It!* / Fowler. See [`todo/phase-3-briefing.md`](./todo/phase-3-briefing.md).
