...
**Status (as of 2026-04-30):** **Phases 1 and 2 complete; Phases 3a, 3b, and inline second-book ingestion complete.** All sub-stages shipped on `feat/multi-book-kg` and pushed to `origin`. Phase 2 verification: `arsanjani_2026.summary` (5145 chars, hybrid Claude-draft + user edit) embedded as 1536-dim vector; `triage_books` MCP tool returns 0.4502 cosine for an obviously-relevant agentic project description and < 0.2 for off-topic ones; 142/142 tests pass. Phase 3a verification: schema (`projects`, `canonical_concepts`, `concept_alignment_cache`) + 11 db helpers + `list_books` MCP tool (27th tool); 23 new tests, 165/165 total passing. Phase 3b verification: `start_project(...)` MCP tool (28th tool) with internal triage fallback and deterministic project_id derivation; 12 new tests, 177/177 total passing. **Inline ingestion verification:** `gulli_2025` (Antonio Gulli, *Agentic Design Patterns*, Springer 2025, altitude=`implementation`) ingested end-to-end — 180 concepts, 390 sections, 802 relationships, summary embedded; concepts schema migrated to composite `UNIQUE(name, book_id)` so the 5 cross-book name collisions (MCP / HITL / LangGraph / CrewAI / ADK) coexist as distinct rows; triage cleanly separates a Gulli-leaning query (gulli wins by +0.04) from an Arsanjani-leaning query (arsanjani wins by +0.07); 181/181 tests pass (4 new schema regression tests in `test_concepts_schema.py`, 4 borderline `cases.py` fixtures pinned to stronger Gulli concept matches). **Next: Phase 3c** — `align_book_pair.py` + `build_project_kg` against the now-real two-book corpus.
