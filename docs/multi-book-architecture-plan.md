...
|---|---|---|
| **1a** | Schema migration: add `books` table, add `book_id NOT NULL` to `concepts` / `sections` / `relationships`, backfill existing rows with `arsanjani_2026`, update `search_concepts_by_embedding` and `get_concept_relationships` to accept optional `book_id` filter | DB integrity check; existing tests still pass |
...
| **1d** | Verification + docs: re-run full pipeline on Arsanjani with `--reset`, confirm all 134 existing tests pass, update `CLAUDE.md` with new schema and pipeline command shape | All tests green; CLAUDE.md current |
