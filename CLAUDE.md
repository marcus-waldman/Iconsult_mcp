...
- `start_project(name, project_description, triaged_book_ids?, project_id?, triage_top_k=5, triage_threshold=0.4)` — START PROJECT: create or refresh a per-project cache row; runs `triage_books` internally when `triaged_book_ids` omitted; deterministic project_id from hash of (name, description) when not user-supplied; idempotent on re-run; does NOT build the unified KG (that is `build_project_kg` in 3c, pending)
